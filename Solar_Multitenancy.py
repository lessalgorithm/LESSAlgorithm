...
# This code is adapted from original code written for the longevity ICDCS paper and subsequent GLOBECOM paper based on solar tracking.
# The propose of this code it to run experiments around adaptive multitenancy driven ENO WSN for an E-WSN submission . 
...
def dfLoad(test):
	df = pd.read_csv('datasets/{}_solarcalc_raw.csv'.format(test),low_memory=False,index_col=0) # change file name to loop
	return df
...
def sysRequirements(df,test,orchest):
	df['Orchastration Requirements'] = orchest * (df.shape[0]/len(orchest)) # This takes the config file dt and multiplies it to be the length of the dataframe in multiples of a day
	return df
...
def panelEnergyGen(df,test):
	...
	E_list=[a*(math.cos(math.radians(c)))+b for a,b,c in zip(DNI_list,DHI_list,Zenith_list)] 
	if debug: print 'E coefficient has been calculated'
	...
	x_list = [a*(math.cos(math.radians(sensor_angle_V-b))) for a,b in zip(DNI_list,Zenith_list)] 

	# energy in w/m2 impinging on the surface of a solar cell
	ES_list = [(a+(b*rd)+(foreground_albedo*c*rr)) for a,b,c in zip(x_list,DHI_list,E_list)] 
	if debug: print 'Energy hitting the cell has been calculated'

	# including temperature as a function of solar cell efficiency in calculations
	temperature_list = getTemplist(df)
	tc_list = [(a+(((Noct-20)/800)*b)) for a,b in zip(temperature_list,ES_list)]
	...
	if debug: print 'Efficiency of the solar cell over time has been calculated'

	# conversion from w/m2 to energy generated by solar panel in mA
	EG_list = [(abs(a*(solar_panel_active_area_m2*b*1000.00))/(solar_panel_voltage)) for a,b in zip(ES_list,efficiency_pvg)] #change ABS here, can't be right
...
def NRELtoWindPower(df):
	...
	wind_speed_list = df["Wind Speed"].tolist() # Making an assumption that as the system is light it will have a fan to point in the direction of the wind
	temperature_list = getTemplist(df)

	air_density_list = [((a*100)/(R_spec*(b+273.15))) for a,b in zip(pressure_list,temperature_list) ] # 100 here convers millibar to pascal and 273.15 is c to kelvin conversion
	...
	for a,b in zip(wind_speed_list,air_density_list):
		if Ve < a and a < Vo:
			temp_power = ((0.5 * b * Area_wind * math.pow(a,3) * cp)/wind_turbine_voltage)
...
def NRELtoTEGPower(df):
	...
	for key in temperature_list:
		...
		if temp > 5:
			vteg_list.append(abs(((N_TEG*seedback_coeff)*(key-T_ambient)/4))) # Check to confirm that the temperature in reverse generates current with neg voltage also and DC/DC can handle that
		else:
			vteg_list.append(0)
...
def calcPerf(df,test,name):
	...
	if storage: output_jsons.append({'source': test, 'test': name, 'Dt_average': average,'variance': varience ,'perTimeDead': dead_metric_per,'perTimeWasted': waste_metric_per ,'orchas' : orchastPlace_list,'sense_freq':sens_freq_list,'orchas_diff': orchas})
...
def staticWSN(df,test):
	if debug: print "working out static current consumption bits"
	DHI_list = df["DHI"].tolist() # Load arbitrary list to iterate about. This will change to input variables when ENO in introduced
	Icons_list = [((((Iq+((Is*Ss)/1800)+((Icomp*Scomp)/1800)+((Itx*Stx)/1800))*sens_freq))*(random.uniform(energy_cons_var[0],energy_cons_var[1]))) for a in DHI_list] # Think about if I'm dividing by 2 before taking from battery is Iq being misquoted	
	currentgen_list = df['Energy Generation Total'].tolist()
	batterylevelflag_list = [2] # This is a simple flag to show when battery is full (2),  battery is empty/system dead (0) or nominally operating (1)
	energygensurplus_list = [0] # This records surplus energy generation that otherwise wouldn't be used when there is no ENO utility stuff
	energydeficit_list = [0] # This list records times of where energy consumption is greater than generation. This is then used to derive battery health
	sens_freq_list = [sens_freq]
	batterylevel_list = [initial_battery_capacity_mah] # This list contains the battery level of the system over the source of the system . Assumes state zero is full battery
	prev = initial_battery_capacity_mah # current battery level. Makes assumption that system starts full
	for a,b in zip(Icons_list[1:],currentgen_list[1:]):
		...
		if x > initial_battery_capacity_mah: # if there is a surplus energy and battery is full
			energygensurplus_list.append(x - initial_battery_capacity_mah) # this is the energy wasted in mAh
			energydeficit_list.append(0) # this appends zero to the deficit list as we have not wasted energy
			batterylevelflag_list.append(2) # appends a 2 to the level flag list
			batterylevel_list.append(initial_battery_capacity_mah) # records the battery level for final calculations
	...
	if debug: print 'Static Current consumption calculated'

# This function calculates the energy consumption of the WSN if only controlled by the MORE algorithm
def orchasWSN(df,test):
	if debug: print "working out MORE current consumption bits"
	orchastPlace_list = df['Orchastration Requirements'].tolist()
	Icons_list = [((((Iq+((Is*Ss)/1800)+((Icomp*Scomp)/1800)+((Itx*Stx)/1800))*a))*(random.uniform(energy_cons_var[0],energy_cons_var[1]))) for a in orchastPlace_list] # Think about if I'm dividing by 2 before taking from battery is Iq being misquoted
	...
	if debug: print 'orchastration Current consumption calculated'
...
def enoWSN(df,test):
	if debug: print "working out ENO current consumption bits"
	...
	currentgen_list = df['Energy Generation Total'].tolist() # Change this to EG_total
	batterylevel_list = [initial_battery_capacity_mah] # This list contains the battery level of the system over the source of the system . Assumes state zero is full battery
	batterylevelflag_list = [2] # This is a simple flag to show when battery is full (2),  battery is empty/system dead (0) or nominally operating (1)
	energygensurplus_list = [0] # This records surplus energy generation that otherwise wouldn't be used when there is no ENO utility stuff
	energydeficit_list = [0] # This list records times of where energy consumption is greater than generation. This is then used to derive battery health
	...
	for a in currentgen_list[1:]:	
		...
		cost_per_tx =  ((((((Is*Ss)/1800)+((Icomp*Scomp)/1800)+((Itx*Stx)/1800))*1))*(random.uniform(energy_cons_var[0],energy_cons_var[1]))) 
		...
		loop += 1
		if loop > Nw-1:
			sanity_check = round(((sum(Eg_Nwi)) - (sum(duty_Nw)*cost_per_tx)),1)  # This checks the number of transmissions over or under utilised + is surplus energy, negative is energy defecit 
...
def lessWSN(df,test):
	if debug: print "working out LESS current consumption bits"
	orchastPlace_list = df['Orchastration Requirements'].tolist()
	Eg_Nwi2 = [0.1]* Nw
	Eg_Nwi  = [0.1] * Nw
	duty_Nw = []
	...
	for a in currentgen_list[1:]:
		...
		sens_freq = min(sens_freq_eno,sens_freq_needed) # Think about this, this needs to choose the orchastrator amount unless that will kill the sustainability of the system
		...
		if x > prev: # This if takes care of the times when battery is full so we don't report greater than 100% storage
			...
			energydeficit_list.append(temp)
			prev = 0
			sens_freq_list.append(0) # Here I could ammend to the amount of tx's they were able to do before the battery died? 
		else: # Normal operating for system, to calc new battery level
			batterylevel_list.append(x)
			batterylevelflag_list.append(1)
			energygensurplus_list.append(0)
		...
		Eg_Nwi[loop] = round(Eg_Nwi[loop],2)
		Eg_Nwi[loop] = a # Have a think about how I abstracted the ENO function with EWMA between N_w and Eg_nwi, should I expand for clarity? 
		...
		if loop == Nw:
			...
			while abs(residual_energy) > (cost_per_tx+(0.5*cost_per_tx)):
				if residual_energy >= cost_per_tx: # If theres a surplus energy 
					diff_1 = 0
					counter = loop
					for a,b in zip(duty_Nw[loop:],orchastPlace_list[loop:]): # This finds the point further along in the loop where the ENO is furthest below the orchestrator						
						diff = b-a 
						if diff > 0: 
							if diff > diff_1:
								index_diff = counter 
								diff_1 = diff
						counter = counter + 1
					if duty_Nw[index_diff] == max_tx_freq: # These don't do anything unless you change pass to break. 
						break
					...
					for a,b in zip(duty_Nw[loop:],orchastPlace_list[loop:]): # This finds the point further along in the loop where the ENO is furthest below the orchestrator
						...
						if diff > 0: 
							if diff > diff_1:
								...
								diff_1 = diff
						counter = counter + 1
					if duty_Nw[index_diff] == min_tx_freq: # These don't do anything unless you change pass to break. 
						break
					duty_Nw[index_diff] = duty_Nw[index_diff] - 1
					residual_energy = residual_energy + cost_per_tx
				else:
...
def dumpData(test):
		if output_jsons:
			...
			resultFile = open("Datasets/test/{}_{}_solartracking_results.json".format(epoch_time,test),'wb')
...
def main():
	...
	for orchest in orchest_loop:
		for test in test_loop:
			...
			if not df.empty:
				if test_loop.index(test) == 0: # Currently only works for one loop because of this line but can be fixed later
					df = sysRequirements(df,test,orchest)
				df = panelEnergyGen(df,test) # Calculates energy generated by solar panel for day
				df = NRELtoWindPower(df) # calculated wind power produciton from environmental variables
				df = NRELtoTEGPower(df) # calculates thermal energy generation from environmental variables
				df = energyGenTotal(df) # calculates energy total by summing the above
				staticWSN(df,test)
